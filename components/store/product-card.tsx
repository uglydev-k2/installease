...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <article className="group rounded-xl border bg-white p-2.5 shadow-sm transition hover:shadow-md md:p-3 dark:bg-slate-900">
      <Link href={`/products/${product.slug}`} className="relative block overflow-hidden rounded-xl">
        <Image
          ...
          width={800}
          height={800}
          className="h-44 w-full object-cover transition duration-300 group-hover:scale-105 md:h-60"
        />
      </Link>
      <div className="mt-3 space-y-2">
        ...
        <div className="flex items-center gap-2">
          <span className="text-base font-bold md:text-lg">${product.salePrice ?? product.price}</span>
          {product.salePrice ? (
